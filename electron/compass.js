...
function buildCompassSnapshot({ globalMarkets = [], domesticMarkets = [], styleMarkets = [], asOf, sourceStatus = {} } = {}) {
  ...
  const coverage = globalMarkets.length + domesticMarkets.length + styleMarkets.length;
  const expected = GLOBAL_MARKETS.length + STYLE_INDEX_DEFINITIONS.length + 3;
